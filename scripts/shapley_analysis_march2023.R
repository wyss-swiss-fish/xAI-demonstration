...
# set the run name to make figures for
RUN <- 'ubelix_test_PARA_RF_v3'

# get species directories
sp_dirs <- list.files(paste0('D:/sdm-pipeline/sdm-run/', RUN), full.names = T)
focal_sp <- c('Alburnoides bipunctatus', 'Barbus barbus', 'Telestes souffia')
sp_dirs <- sp_dirs[grepl(paste0(focal_sp, collapse = '|'), sp_dirs)]

# get the path to environmental data for all species
sp_env_path <- paste0(sp_dirs, '/data/env_data.TIF')

# get path to data used to fit models
sp_data_po <- paste0(sp_dirs, '/data/sdm_input_data_rf_poFinal.rds')
sp_data_pa <- paste0(sp_dirs, '/data/sdm_input_data_rf.rds')

# get path to species distribution models rasters
sp_raster_suit_po <- paste0(sp_dirs, '/output/raster_maps/suitability_stack_po_rf.TIF')
#sp_raster_pres_po <- paste0(sp_dirs, '/output/raster_maps/presence_stack_po_rf.TIF')
sp_raster_suit_pa <- paste0(sp_dirs, '/output/raster_maps/suitability_stack_pa_rf.TIF')
#sp_raster_pres_pa <- paste0(sp_dirs, '/output/raster_maps/presence_stack_pa_rf.TIF')

# get paths to secies distribution model object
sp_RF_po <- paste0(sp_dirs, '/output/final_models_rf_po.RDS')
sp_RF_pa <- paste0(sp_dirs, '/output/final_models_rf_pa.RDS')
...
record_table <- record_table %>% filter(species_name %in% focal_sp)

# check we are working with the same length of runs
nrow(record_table) == length(sp_dirs)

#### Read in model for each species and estimate shapley value ----

# for(i in 1:length(sp_dirs)){
i = 1

if (!file.exists(sp_RF_po[i])) {
  next()
}

# add logic here to define what data is available for this modelling run
fitting <- c("presence_only", "presence_absence")[c(record_table$evaluate_po[i], record_table$evaluate_pa[i])]

# first aggregate suitabilities to catchments to speed up estimation of shapley values
if ("presence_only" %in% fitting) {
  
  ## get PO raster
  rast_po <- mean(rast(sp_raster_suit_po[i]), na.rm = T)
  
  ## extract suitability across polygons
  po_poly <- terra::extract(x = rast_po, 
  ...
  subcatchments_suit_po <- cbind(subcatchments_final, po_poly)
  subcatchments_suit_po_df <- st_drop_geometry(subcatchments_suit_po) %>% na.omit()
  
  print(tm_shape(subcatchments_suit_po) + 
          tm_polygons(col = 'mean', palette = 'viridis', style = 'cont', breaks=c(0, 0.2, 0.4, 0.6, 0.8, 1), 
                      border.col = 'transparent', midpoint = NA, title = "", legend.reverse = T) +
          tm_shape(river_intersect_lakes) +
          tm_lines() + 
          tm_shape(lakes) + 
          tm_polygons(border.col = 'black', col = 'gray95', legend.show = F) + 
  ...
  env_poly <- terra::extract(x = rast(sp_env_path[i]), 
                             y = vect(subcatchments_final), # catchment data produced in the spatial_data.R script 
                             fun = function(x){mean(x, na.rm = T)})
  ...
  sp_RF_po_i <- readRDS(sp_RF_po[i])
  sp_data_po_i <- readRDS(sp_data_po[i])$rf
  
  # run shapleys over all model iterations
  sp_shapley <- lapply(1:length(sp_RF_po_i), function(x){ 
    
    vars <- colnames(attr(sp_RF_po_i[[x]]$terms,"factors"))
    
    get_shapleys(model = sp_RF_po_i[[x]], # random forest model fitted to data
                 data = sp_data_po_i[[x]],  # data used to fit the random forest
